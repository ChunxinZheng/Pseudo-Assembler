...
;;    FIND ALL SYMBOLS && GENERATE ADDITIONAL INSTRUCTIONS
;; ---------------------------------------------------------------------

(struct result (prog table)  #:transparent #:mutable)


;; (first-scan aprimpl empty 0 empty), reversed
...
;           [`(lit ,psymbol)
;            (first-scan (rest lst) (cons psymbol acc) count table)]
           [`(data ,psymbol (,nat ,val))
;;            (define c (cond [(symbol? val) val] [else count]))
            (first-scan (rest lst) (cons (first lst) acc)
                        (+ nat count) (cons (make-data psymbol count) table))]
           [`(data ,psymbol ...)
;            (display psymbol)
;            (display (first psymbol))
;;            (define c (cond [(symbol? (second psymbol)) (second psymbol)] [else count]))
            (first-scan (rest lst)
                        (cons (first lst) acc)
;                        (append (foldl (lambda (x l) (cons (list x) l))
;                                       empty (rest (rest (first lst)))) acc)
...
(define (make-psymbol-hash-table psymbols)
  (define (h psymbols table)
    (define (name) (psymbol-name (car psymbols))) ;; make name a thunk to stop it from evaluating when psybols is empty
    (cond
     [(empty? psymbols) table]
     [(not (hash-ref-key table (name) #f)) (h (cdr psymbols) (hash-set table (name) (car psymbols)))]
  ...
  (define (h sym-table env cur stack defs is_base)
    ...
    (cond
      [(not (eq? 'undef cur))
       ...
       (if (lit? val) (h (hash-set sym-table name (psymbol type name val)) env 'undef stack defs is_base)
           (let
               ([stored-val (hash-ref sym-table val #f)]
                [traced-val (hash-ref env       val #f)])
             (cond
               [stored-val (h (hash-set sym-table name (psymbol type name (psymbol-val stored-val))) env 'undef stack defs #t)]
               [traced-val
      ...
      [(not (empty? stack )) (h sym-table (first (first stack)) (second (first stack)) (cdr stack) defs #t)]
  ...
  (cond
    [(symbol? symbol)
     ...
     (cond
         [(const? psym) val]
         [(data?  psym) val]
         [else (incorrect-usage-error (format "cannot use ~a as the first value of an indexed access" symbol))])]
    [(lit? symbol) symbol]
    [else symbol]));; remove this if we want to check this error
    ;;[else (incorrect-usage-error (format "cannot use ~a as the first value of an indexed access" symbol))]))
  ...
  (cond
    [(symbol? symbol)
     ...
     (cond 
         [(const? psym) (list val)]
         [(data?  psym) (list val)]
         [else (incorrect-usage-error (format "cannot use ~a as the second value of an indexed access" symbol))])]
    [(list? symbol) symbol]
    [else symbol]));; remove this if we want to check this error
    ;;[else (incorrect-usage-error (format "cannot use ~a as the second value of an indexed access" symbol))]))
...
(define (resolve-lit-data-const table symbol)
  (cond
    ...
    [(lit? symbol) symbol]
    [else symbol]));; remove this if we want to check this error
    ;;[else (incorrect-usage-error (format "cannot use ~a as a lit/data/const" symbol))]))
  ...
  (cond
    [(symbol? symbol)
     ...
     (cond 
         ...
         [(data?  psym) (list val)]
         [(label? psym)       val ]
         [else (incorrect-usage-error (format "cannot use ~a as a the destination of a jump/branchs" symbol))])]
    [(lit? symbol) symbol]
    [else symbol]));; remove this if we want to check this error
    ;;[else (incorrect-usage-error (format "cannot use ~a as a the destination of a jump/branch" symbol))]))
...
(define (resolve-opd table symbol)
  (match symbol
    ...
    [x symbol]));; remove this if we want to check this error
    ;;[x (incorrect-usage-error (format "cannot use ~a as a the destination of a jump/branch" symbol))]))
...
(define (primpl-assemble aprimpl)
  ...
  ;;(display prog-with-psymbols)
  ;;(newline)

  ;(display psymbol-list)
  ;(newline)
  (define psymbol-table (generate-psymbol-table psymbol-list))
  ;(display psymbol-table)
  ;(newline)
